...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [h_S,delta_S,zeta_S,T_sunrise,T_sunset,L_day,jDay] = SetSunVariables(Datam,DeltaGMT,Lon,Lat,t_bef,t_aft)
% [h_S,delta_S,zeta_S,T_sunrise,T_sunset,L_day,jDay] = SetSunVariables([2015,6,21,10],-5,74.6507,40.3534,0.5,0.5)
%%% INPUT 
%%% Datam %% [Yr, MO, DA, HR]
%%% DeltaGMT [h] difference with greenwhich Meridian Time
%%% Lon [°]
%%% Lat [°]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% OUTPUT 
...
nowYR=Datam(1); nowMO=Datam(2); nowDA=Datam(3); nowHR =Datam(4)+ Datam(5)/60 + Datam(6)/3600;    
if(nowMO==1)
    jDay = nowDA;
elseif(nowMO==2)
    jDay = days(1) + nowDA;
