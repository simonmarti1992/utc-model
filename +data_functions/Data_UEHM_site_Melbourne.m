...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Height_canyon	=	6.4;	% Canyon (building) height (m)
Width_canyon	=	15.2;	% Canyon (road) width (m)
Width_roof		=	12.2;	% Roof width (m), calculated from the land cover fraction and the street width.
Height_tree		=	4.2;	% Tree height (m)
Radius_tree		=	1.5;	% Tree-crown radius (m), Calculated out of rescaled tree fraction and street width (assuming two uniform strips of tree rows)
Distance_tree	=	2;		% Tree-to-wall distance (m), just a guess, I don't know.

trees	=	1;		% If trees are present in the urban canyon: 1, if no trees are present: 0
ftree	=	1;		% Tree fraction along canyon axis (Always put 1)
...
% GROUND %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fveg_G	=	0.45;	% Partitioning ground vegetation surface(-)
fbare_G	=	0.02;	% Partitioning ground bare surface(-)
fimp_G	=	0.53;	% Partitioning ground impervious(-)
...
eimp_R		=	0.92;	% Roof impervious emissivity (-)
...
eimp_G		=	0.92;	% Ground impervious emissivity (-)
...
emissivity_W	=	0.88;	% Wall emissivity (-)
...
lan_dry_imp_R	=	0.773;			% Thermal conductivity dry solid [W/m K]
cv_s_imp_R		=	0.813*10^6;		% Volumetric heat capacity solid [J/m^3 K]

ParThermalRoof	=	struct('lan_dry_imp',lan_dry_imp_R,'cv_s_imp',cv_s_imp_R);

% GROUND THERMAL PROPERTIES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lan_dry_imp_G	=	2.682;			% Thermal conductivity dry solid [W/m K], asphalt
cv_s_imp_G		=	1.3413*10^6;	% Volumetric heat capacity solid [J/m^3 K], asphalt

ParThermalGround	=	struct('lan_dry_imp',lan_dry_imp_G,'cv_s_imp',cv_s_imp_G);

% WALL THERMAL PROPERTIES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lan_dry_W	=	0.342;				% Thermal conductivity dry solid [W/m K]
cv_s_W		=	0.9035*10^6;		% Volumetric heat capacity solid [J/m^3 K]
...
ZR95_R		=	1;		% Root depth 95 percentile [mm]
...
Sl_R		=	0.035;	% 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
% GRASS
% General
LAI_G		=	3;			% Leaf area index of ground vegetation (-)
...
ZR95_G		=	200;	% Root depth 95 percentile [mm]
ZR50_G		=	150;	% Root depth 50 percentile [mm]
ZRmax_G		=	300;	% Maximum Root depth [mm]
Rrootl_G	=	4500;	% Root length index [m root / m^2 PFT]
PsiL50_G	=	-2.5;	% [MPa]  Water Potential at 50% loss conductivity
PsiX50_G	=	-9.5;	% Water potential at 50 of xylem hydraulic conductivity and limit for water extraction from soil
...
a1_G		=	7;		% [-] Empirical parameter connecting stomatal aperture and net assimilaton
...
Psi_sto_00_G=	-0.6;	% [MPa]  Water Potential at PLCs loss conductivity
Psi_sto_50_G=	-2;		% [MPa]  Water Potential at 50% loss conductivity
Sl_G		=	0.016;	% [m^2 gC] specific leaf area of  biomass
...
% TREE VEGETATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% General
LAI_T		=	3;		% Leaf area index for the ground vegetation (-) (Liu et al. 2017)
SAI_T		=	0.1;	% Stem area index for the ground vegetation (-)
d_leaf_T	=	3;		% Leaf dimension of ground vegetation [cm]

% Roof water uptake
CASE_ROOT_T	=	1;		% Type of Root Profile
ZR95_T		=	1000;	% Root depth 95 percentile [mm]   (Harshan et al 2017)
ZR50_T		=	350;	% Root depth 50 percentile [mm]
ZRmax_T		=	1900;	% Maximum Root depth [mm]
Rrootl_T	=	5000;	% Root length index [m root / m^2 PFT]
PsiL50_T	=	-2.5;	% [MPa]  Water Potential at 50% loss conductivity
PsiX50_T	=	-9;		% Water potential at 50 of xylem hydraulic conductivity and limit for water extraction from soil

% Photosynthesis and Transpiration
FI_T		=	0.081;	% Intrinsec quantum Efficiency [umolCO2/umolPhotons]
Do_T		=	1200;	% [Pa] Empirical coefficient for the role of vapor pressure in the biochemical model of photosynthesis
a1_T		=	8;		% [-] Empirical parameter connecting stomatal aperture and net assimilaton
...
Knit_T		=	0.15;	% [-] Canopy nitrogen decay coefficient
...
Psi_sto_00_T=	-0.7;	% [MPa]  Water Potential at PLCs loss conductivity
Psi_sto_50_T=	-1.5;	% [MPa]  Water Potential at 50% loss conductivity
Sl_T		=	0.009;	% 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
% ROOF %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Layer thickness
dz1_R	=	0.05705;       % Thickness of first roof layer [m]
dz2_R	=	0.05705;       % Thickness of second roof layer [m]

% Soil layer discretization 
Zs_R	=	[ 0 10 20 30 57.07];% soil layer discretization [mm]
...
% WALL %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Layer thickness %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dz1_W	=	0.07445;       % Thickness of first wall layer [m]
dz2_W	=	0.07445;       % Thickness of second wall layer [m]
